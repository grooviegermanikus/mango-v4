    ...
    let poll_buy_price = tokio::spawn({
        async move {
            ...
            loop {
                ...
                // TODO use it!
                let price2 = services::asset_price_swap_buy::call_buy_eth().await;
                println!("buy price for usd {:?}", price2);

                println!("diff {}", (price.price - price2.price).abs());
    ...
    let main_poller = tokio::spawn({
        ...
        async move {
            ...
            loop {
                ...
                info!("orderbook(perp) best bid {:?}", *orderbook_bid);

                let orderbook_ask = last_ask_price.read().await;
                info!("orderbook(perp) best ask {:?}", *orderbook_ask);

                if let (Some(perp_bid), Some(swap_buy)) = (*orderbook_bid, latest_buy) {
                    info!("perp-bid {:?} vs swap-buy {:?}, profit {:.2?}%", perp_bid, swap_buy.price, 100.0 * (perp_bid - swap_buy.price) / swap_buy.price);
                }
